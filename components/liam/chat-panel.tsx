    ...
    <div className="flex flex-col h-full bg-[#050505]">
      ...
      <div className="flex items-center gap-3 p-4 border-b border-white/10 bg-black/30">
        <div className="w-10 h-10 rounded-full bg-[#26FC00]/10 border border-[#26FC00]/30 flex items-center justify-center flex-shrink-0">
          <img src="/ic-bird.svg" alt="Liam" className="w-6 h-6" style={{ filter: "invert(68%) sepia(98%) saturate(426%) hue-rotate(56deg) brightness(103%) contrast(101%)" }} />
        </div>
        <div className="min-w-0">
          <p className="font-sans font-semibold text-white text-base tracking-tight">Liam AI</p>
          <p className="font-mono text-[9px] tracking-[0.2em] text-[#26FC00] uppercase">● Online · IC Vacation Consultant</p>
        </div>
          ...
          <div key={msg.id} className={`flex gap-3 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
            {msg.role === "assistant" && (
              <div className="w-7 h-7 flex-shrink-0 rounded-full bg-[#26FC00]/10 border border-[#26FC00]/30 flex items-center justify-center mt-0.5">
                <img src="/ic-bird.svg" alt="" className="w-4 h-4" style={{ filter: "invert(68%) sepia(98%) saturate(426%) hue-rotate(56deg) brightness(103%) contrast(101%)" }} />
              </div>
            )}
            <div
              className={`max-w-[82%] ${
                msg.role === "user"
                  ? "bg-[#26FC00]/10 border border-[#26FC00]/20 px-4 py-3"
                  : "bg-black/40 border border-white/10 px-4 py-3"
              }`}
            >
              <p className="font-serif text-[15px] text-white/85 leading-[1.65] whitespace-pre-wrap">
                {msg.content}
      ...
      {/* CTA banner */}
      {showCta && (
        <div className="mx-4 mb-3 p-3 border border-[#FFE500]/20 bg-[#FFE500]/5 flex items-center justify-between gap-3">
          <div className="min-w-0">
            <p className="font-mono text-[10px] tracking-[0.15em] text-[#FFE500] uppercase">Ready to go deeper?</p>
            <p className="font-serif italic text-sm text-white/60 mt-0.5">Isaac personally reviews every brief.</p>
          </div>
          <a
            href="tel:+14078101670"
      ...
      <div className="p-4 border-t border-white/10 bg-black/30">
        <div className="flex gap-2 items-end">
          <textarea
            className="flex-1 bg-[#050505] border border-white/15 px-3 py-2.5 text-sm font-serif text-white/80 placeholder:text-white/25 focus:outline-none focus:border-[#26FC00] resize-none leading-relaxed"
